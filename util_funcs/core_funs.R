...
library("moments")
library("MASS")
# library("rmutil") # package to generate 'lognormal', 'burr', 'generalized gamma'. #
#library("mixR") # package to generate mixture gamma #
rmixgamma.simul <- function(n,pi1=0.4,a1=0.5,b1=0.5,a2=5,b2=1){
  # Sampling from the mixture - no loop
  n1=rbinom(1,n,pi1)
  n2=n-n1
  x1=rgamma(n1,shape=a1,scale=b1)
  x2=rgamma(n2,shape=a2,scale=b2)
  X= c(x1,x2)  #combine the two samples
  X= X[sample(1:n)]  # mix up the rows
  return(X)   
}

rmix.simul <- function(n,pi1=0.3,a1=0.5,b1=2.5,a2=0.5,b2=1){
  # Sampling from the mixture - no loop
  n1=rbinom(1,n,pi1)
  n2=n-n1
  x1=rgamma(n1,shape=a1,scale=b1)
  #x2=rgamma(n2,shape=a2,scale=b2)
  x2 = rlnorm(n2, meanlog = a2, sdlog = b2) 
  X= c(x1,x2)  #combine the two samples
  X= X[sample(1:n)]  # mix up the rows
  return(X)   
}
...
fsim.logacd <- function (n,nb,omega,alpha,beta,feps,par1=1,par2=1) 
{
  ...
  { cat("Error: The simulated process is not weakly stationary")
    ...
    } else if (feps=="lognormal") {  	
      ## Generate errors from lognoraml distribution			
      eps <- eps.lognorm <- rlnorm(nt, meanlog = par1, sdlog = par2) 	 
    } else if (feps=="burr") {    
      ## Generate errors from Burr distribution			
      eps <- eps.burr <- rburr(nt, m = par1, s = par2, f=2) 	 
    } else if (feps=="ggamma") {    
      ## Generate errors from generalized gamma distribution  		
      eps <- eps.ggamma <- rggamma(nt, s = par1, m = par2, f=2) 	 
    } else if (feps=="mixgamma") {    
      ## Generate errors from mix gamma distribution    	
      eps <- eps.mgamma <- rmixgamma.simul(nt)	 
    } else if (feps=="mix") {    
      ## Generate errors from mix  distribution    	
      eps <- eps.mgamma <- rmix.simul(nt)	 
    }
